...
while planes!=():
    ...
    for i in range(cant):
        ...
        dir1= os.path.abspath("/home/futbol/public_html/codes/"+str(stactic1[0][2]))
        dir2= os.path.abspath("/home/futbol/public_html/codes/"+str(stactic2[0][2]))
        ...
        for i in range(cant):
            if juegos[i].getTiempo()>0:
                ...
                if done[i]== False:
                    ...
                    tarjeta= file("../games/"+cadena+".txt","w")
                    tarjeta.write(str(planes[i][0])+","+str(planes[i][1])+","+str(gol1)+","+str(gol2)+","+str(distie)+","+cadena+","+str(time.time())+"\n")
                    tarjeta.write("id1,id2,gol1,gol2,distie,idGame,tiempo")
                    tarjeta.close()
                    tarjeta="../games/"+cadena+".txt"
                    
                    rar=os.path.abspath("../games/"+cadena+".zip")
                    fer="../games/"+cadena+".fer"
